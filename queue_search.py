...
from bbSearch2022.tree import * # defines tree data structure
...
def search(problem, strategy, max_nodes, options ):
    ...
    problem.info()  # Call function to print problem info
    ...
    node_queue = [node_set_state( new_node(), problem.initial_state )]
    ...
    for nodes_tested in range(max_nodes):
          ...
          if problem.goal_test( node_get_state( first_node ) ):   
             action_path = node_get_path(first_node)
             print( "\n:-)) *SUCCESS* ((-:" )
             print( "The action/state path to the solution is:" )
             #print_action_list(action_path)
             print_state_action_sequence( problem, action_path )
             print( "Path length = " + str( len(action_path) ) )
             print( "Total nodes tested = " + str(nodes_tested+1) )
             print( "Time taken =", time.perf_counter() - start_time, "seconds\n" )
             return action_path
          children = node_get_children( node_expand( first_node, problem ) )
...
def node_expand(node, problem ):

      state = node_get_state( node )
      path  = node_get_path( node )
      poss_acts = problem.possible_actions(state)
      suc_pairs = [(act, problem.successor(state,act)) for act in poss_acts]
...
def print_state_action_sequence( problem, action_list ):
    state = problem.initial_state
    problem.display_state(state)
    for act in action_list:
        problem.display_action( act )
        state = problem.successor( state, act )
        problem.display_state(state)
...
#jsp = JugsSearchProblem()
#search( jsp, 'breadth_first', 1000, [] )
